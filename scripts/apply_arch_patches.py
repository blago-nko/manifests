#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Универсальный патчер манифестов БЛАГО-НКО (v2).

Режимы:
- GitHub Actions: только применяет патчи; ветку/коммит/PR создаёт workflow.
- Codespaces / локально: сам делает ветку + коммит + push + ссылку на PR.

Выбор протокола: переменная окружения PATCH:
  all | SAM-INFRA-011 | SAM-LIC-007 | SAM-LIC-008 |
  SAM-INFRA-012 | SAM-MIG-005 | SAM-INFRA-013
"""
import re, sys, subprocess, os

PATCH = os.environ.get("PATCH", "all")
IN_ACTIONS = os.environ.get("GITHUB_ACTIONS") == "true"
BRANCH = "fix/architecture-consistency-2026-08-13"

PATCHES = [
{"proto":"SAM-INFRA-011","file":"ЛИЦ.md","type":"lit","required":True,
 ...
 "desc":"ЛИЦ заголовок: CC BY-SA -> CC BY"},
{"proto":"SAM-LIC-008","file":"ЛИЦ.md","type":"lit","required":True,
 ...
 "desc":"ЛИЦ: защита витрины САН -> CC BY-NC-ND"},
{"proto":"SAM-LIC-007","file":"САМ.md","type":"lit","required":True,
 ...
 "desc":"САМ 1.14: ноу-хау -> copyright"},
{"proto":"SAM-INFRA-012","file":"САМ.md","type":"lit","required":True,
 ...
 "desc":"САМ 4.9: унификация S3-хранилищ"},
{"proto":"SAM-INFRA-013","file":"САМ.md","type":"regex","required":True,
 ...
 "desc":"САМ В2: контактный email -> webmaster + fallback"},
{"proto":"SAM-INFRA-013","file":"САМ.md","type":"regex","required":True,
 "old":r'(\|\s*Техническая поддержка\s*\|\s*)`?blagorussia@yandex\.ru`?(\s*\|)',
 "new":r'\1`webmaster@blagorussia.ru` ⚠️ До завершения Миграционного Этапа 1 fallback: `blagorussia@yandex.ru`\2',
 "desc":"САМ В2: техподдержка email -> webmaster + fallback"},
{"proto":"SAM-INFRA-013","file":"САМ.md","type":"regex","required":True,
 ...
 "desc":"САМ Г5: email для 152-ФЗ -> webmaster + fallback"},
{"proto":"SAM-INFRA-013","file":"СУМКа.md","type":"regex","required":True,
 ...
 "desc":"СУМКа 8.2: пометка активации webmaster"},
{"proto":"SAM-MIG-005","file":"МИГРАЦИЯ.md","type":"regex","required":True,
 ...
 "desc":"МИГРАЦИЯ 5.1.4: изоляция САН на Vercel ч.1"},
{"proto":"SAM-MIG-005","file":"МИГРАЦИЯ.md","type":"regex","required":True,
 "old":r'\s*вместе с остальными 13 контентными сайтами',
 "new":"",
 "desc":"МИГРАЦИЯ 5.1.4: убрать совместный деплой ч.2"},
{"proto":"SAM-MIG-005","file":"МИГРАЦИЯ.md","type":"regex","required":True,
 "old":r'(DNS для `?can\.blagorussia\.ru`? временно смотрит на )GitHub Pages',
 "new":r'\1Vercel',
 "desc":"МИГРАЦИЯ 5.1.4: DNS -> Vercel ч.3"},
]
...
def main():
    targets = [p for p in PATCHES if PATCH == "all" or p["proto"] == PATCH]
    if not targets:
        print(f"❌ Неизвестный протокол: {PATCH}"); sys.exit(1)

    files = sorted(set(p["file"] for p in targets))
    for f in files:
    ...
    if os.path.exists("scripts/sync_manifests.py"):
        ...
        r = subprocess.run([sys.executable, "scripts/sync_manifests.py"],
                           capture_output=True, text=True)
        print(r.stdout[-1500:])
        if r.returncode != 0:
            print("⚠️ sync ошибка:\n", r.stderr[-1500:])

    # --- режим GitHub Actions: git делает workflow ---
    if IN_ACTIONS:
        print("\n✅ Патчи применены. Ветку/коммит/PR создаст workflow.")
        return

    # --- режим Codespaces / локально: git сами ---
    ...
    run("git", "add", "-A")
    run("git", "commit", "-m", f"fix(manifests): патч {PATCH}\n\nSAM-INFRA-011/012/013, SAM-LIC-007/008, SAM-MIG-005")
