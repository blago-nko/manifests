...
## Раздел 7. Медиа-Архитектура (Blogger Storage)

> Согласно САМ (п. 2.4 «Эволюционная модель хранения») и МИГРАЦИЯ (п. 5.1.1.1):

- **Основное хранилище изображений:** Blogger CDN.
- **Ограничение размера:** Ресайз ≤ 1600px по большей стороне.
- **Локальное хранение:** Запрещено для боевых сайтов (только staging/backup).
- **R2 Usage:** Только для PDF-документов и медиа-паспортов (`media_passport.json.gz`).
- **Script:** `scripts/image_sanitizer.py` отвечает за скачивание, очистку EXIF, ресайз и загрузку обратно в Blogger API.

| Артефакт СУМКа | Реализация в shared-assets | Где живёт | Когда | Статус |
| :-- | :-- | :-- | :-- | :-- |
| 7.1 Blogger Image Storage | Шорткод {{< img >}} генерирует URL из params.blogger_id | hugo/layouts/shortcodes/img.html | Этап 2 (пилот) | ⬜ |
| 7.2 Media Passport | Генерация JSON-LD ImageObject с авторством и лицензией | partials/jsonld-image.html | Этап 2 (пилот) | ⬜ |
